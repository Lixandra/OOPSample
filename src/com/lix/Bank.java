...
public class Bank {
		...
		for (int i = 0; i < personList.size(); i++) {
			if (personList.get(i).getId() == idPerson) {
				...
				for (int j = 0; j < listAccountPerson.size(); j++) {
					if (((listAccountPerson.get(j) instanceof Saving) && (accTypeSelectedparam == 1))
							|| ((listAccountPerson.get(j) instanceof Checking) && (accTypeSelectedparam == 2))
							|| ((listAccountPerson.get(j) instanceof Credit) && (accTypeSelectedparam == 3))) {// CHECK
	...
	public void addOperationToAccount(int idPerson, int selection, Transactions kindTransaction, double myamount,
			String descrip, Date transacDate) throws Exception {
		boolean flag = false;
		boolean noAcc = false;
		for (int i = 0; i < personList.size(); i++) {
			if (personList.get(i).getId() == idPerson) {
				...
				for (int j = 0; j < mAcc.size(); j++) {
					if (((mAcc.get(j) instanceof Saving) && (selection == 1))
							|| ((mAcc.get(j) instanceof Checking) && (selection == 2))
							|| ((mAcc.get(j) instanceof Credit) && (selection == 3))) {
						// applying interest
						...
						noAcc = true;
						personList.get(i).getAccounts().get(j).addOperations(myamount, kindTransaction, descrip,
								transacDate);
		...
		for (int i = 0; i < personList.size(); i++) {
			...
			if ((personList.get(i).getId() == idPerson) && (accSize != 0)) {
				for (int j = 0; j < accSize; j++) {
					Status s = new Status(personList.get(i).getAccounts().get(j),
							personList.get(i).getAccounts().get(j).getAccoutAmount());
	...
	public List<Person> clientsMostDebt() throws Exception {
		...
		int k = 2;// this is to check the list of person the 2nd time to find
					// people with amount == to biggest
		while (k != 0) {
			for (int i = 0; i < aux.size(); i++) {
				for (int j = 0; j < aux.get(i).getAccounts().size(); j++) {

					if ((aux.get(i).getAccounts().get(j) instanceof Credit)
							&& ((((Credit) (aux.get(i).getAccounts().get(j))).getLimit()
									- aux.get(i).getAccounts().get(j).getAccoutAmount()) >= biggest)
							&& (k != 1) && (aux.get(i).getAccounts().get(j).getAccoutAmount() != 0)) {
						biggest = ((Credit) (aux.get(i).getAccounts().get(j))).getLimit()
								- aux.get(i).getAccounts().get(j).getAccoutAmount();
						// p = aux.get(i);
						break;
					} else if ((aux.get(i).getAccounts().get(j) instanceof Credit)
							&& ((((Credit) (aux.get(i).getAccounts().get(j))).getLimit()
									- aux.get(i).getAccounts().get(j).getAccoutAmount()) == biggest)
							&& (k == 1) && (aux.get(i).getAccounts().get(j).getAccoutAmount() != 0)) {
						aux2.add(aux.get(i));
	...
	/*
	 * public List<Person> clientsMostDebt() throws Exception{ List<Person> aux
	 * = new ArrayList<>(); for (int i = 0; i < personList.size(); i++) { for
	 * (int j = 0; j < personList.get(i).getAccounts().size(); j++) {
	 * if(personList.get(i).getAccounts().get(j) instanceof Credit){
	 * aux.add(personList.get(i)); break; } } } double biggest = 0; Person p =
	 * new Person(); if(aux.size()!=0){ for (int i = 0; i < aux.size(); i++) {
	 * for (int j = 0; j < aux.get(i).getAccounts().size(); j++) {
	 * if((aux.get(i).getAccounts().get(j) instanceof Credit) &&
	 * ((((Credit)(aux.get(i).getAccounts().get(j))).getLimit() -
	 * aux.get(i).getAccounts().get(j).getAccoutAmount()) >= biggest) &&
	 * (aux.get(i).getAccounts().get(j).getAccoutAmount() != 0)){ biggest =
	 * ((Credit)(aux.get(i).getAccounts().get(j))).getLimit() -
	 * aux.get(i).getAccounts().get(j).getAccoutAmount(); p = aux.get(i); break;
	 * } } } List<Person> result = new ArrayList<>(); if(biggest != 0)
	 * result.add(p); else throw new
	 * Exception("CLients with CC may not have perform any transaction"); } else
	 * throw new
	 * Exception("Any registered client has Credit account or they does not have any debt"
	 * ); return }
	 */
	...
	public List<Person> clientMostMoney() {
		...
		for (int i = 0; i < personList.size(); i++) {
			sum = 0;
			for (int j = 0; j < personList.get(i).getAccounts().size(); j++) {
				if ((personList.get(i).getAccounts().get(j) instanceof Checking)
						|| (personList.get(i).getAccounts().get(j) instanceof Saving)) {
